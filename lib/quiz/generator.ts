...
// OpenRouter's free router only selects models whose prompt and completion price is $0.
...
async function evidence(seen:{answer_key:string}[]):Promise<Evidence[]>{
 ...
 const choices=topics.filter(t=>!keys.has(normalize(t))).map(t=>({t,r:Math.random()})).sort((a,b)=>a.r-b.r).slice(0,24).map(x=>x.t);
 ...
 return Object.values(data.query?.pages||{}).filter(p=>p.extract&&p.extract.length>600).map(p=>({title:p.title,url:p.fullurl,text:p.extract!}));
}
async function respond(system:string,input:string){
 ...
 const response=await fetch(`${OPENROUTER_URL}/chat/completions`,{
  ...
  body:JSON.stringify({model:MODEL,messages:[{role:'system',content:system},{role:'user',content:input}],response_format:{type:'json_object'},temperature:.8,max_tokens:13000})
 });
